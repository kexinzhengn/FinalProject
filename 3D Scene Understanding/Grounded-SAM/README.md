...
# Modification notes for ISDN5300
This is an adapted version of [RAM-Grounded-SAM](https://github.com/IDEA-Research/Grounded-Segment-Anything), modified for use with image sequences from the SLAM dataset, such as ScanNet sequences. Key changes include:

- Enhancement of text prompts: Recent label detections are integrated into the text prompts sent to GroundingDINO.
- In GroundingDINO, each grounded phrase is individually decoded into label measurements, with the associated similarity scores for each label being preserved.
- Addition of program interfaces, like ```frame_gap```, ```split_file```, etc.

You can download the pre-trained model [here](https://drive.google.com/drive/folders/1QBIrZ4vKTLhvT_LdQ4cXZZPdd0LTzFpk). Download the file ```checkpoints.zip``` and unzip and put it in the directory as ```./checkpoints```.

Please follow the official instruction of the Installation```Install without Docker``` and install Segment Anything, Grounding DINO,  RAM & Tag2Text. To segment an image sequence, 
```
sh ram_demo_noaug_scannet.sh scans val
```
Please make sure that the ```ScanNet``` folder is in the directory as ```../data2/ScanNet```. The output will be saved in ```../data2/ScanNet/scans/scene0011_01/prediction_no_augment```.
